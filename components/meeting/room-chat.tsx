...
export default function RoomChat({ 
  ...
  const handleAiChat = async (message: string) => {
    ...
    setAiChatHistory(prev => [...prev, aiPlaceholder]);

    try {
      // Send message using AiContextManager with streaming
      await aiContextManager.sendAiMessage(message, {
        roomName,
          ...
          setAiChatHistory(prev => 
            prev.map(msg => 
              msg.id === aiPlaceholder.id 
                ? { ...msg, message: msg.message + chunk.content }
                : msg
            )
          );
        } else if (chunk.type === 'context') {
          // Update with context metadata
          setAiChatHistory(prev => 
            prev.map(msg => 
              msg.id === aiPlaceholder.id 
                ? { 
